/**
 * 工作室固定支出（按月）：筛选、按月分组、汇总、复制清单
 * UTF-8
 */

import { getStudioExpenses, setStudioExpenses, nextId } from '../../utils/storage'
import { drainPendingCloudBackup, syncStudioExpensesAfterLocalChange } from '../../utils/cloud'
...
type MonthGroup = {
  yearMonth: string
  yearMonthDisplay: string
  monthSubtotal: string
  itemCount: number
  items: ItemRow[]
}

function padYmFromDatePick(v: string): string {
...
function sortExpenses(items: StudioExpense[]): StudioExpense[] {
...
function toItemRow(e: StudioExpense): ItemRow {
  return {
    ...e,
...
function buildMonthGroups(filtered: StudioExpense[]): MonthGroup[] {
  const sorted = sortExpenses(filtered)
  const map = new Map<string, StudioExpense[]>()
...
function collectFilterYears(all: StudioExpense[]): number[] {
...
function matchesSearch(e: StudioExpense, q: string): boolean {
  if (!q) return true
...
function runAfterStudioExpenseSyncToast(): void {
...
Page({
  data: {
    ...
    summarySub: '',
    searchQuery: '',
    filterLabels: ['全部'] as string[],
    filterValues: [-1] as number[],
    filterYearIndex: 0,
  },
  ...
  onFilterYearPick(e: WechatMiniprogram.PickerChange) {
    playTouchSound()
  ...
  onSearchInput(e: WechatMiniprogram.Input) {
    this.setData({ searchQuery: (e.detail.value as string) || '' })
    this.loadList()
  },
  ...
  onCopySummary() {
    playTouchSound()
    const groups = this.data.monthGroups as MonthGroup[]
    const labels = this.data.filterLabels as string[]
    ...
    const lines: string[] = ['【工作室支出清单】', `范围：${scope}${q ? ` · 搜索「${q}」` : ''}`, '']
    let n = 0
  ...
  onFormMonthChange(e: WechatMiniprogram.PickerChange) {
    const ym = padYmFromDatePick(String(e.detail.value || ''))
    if (ym) this.setData({ formYearMonth: ym })
  },

  onFormAmountInput(e: WechatMiniprogram.CustomEvent) {
    this.setData({ formAmount: (e.detail.value as string) || '' })
  },

  onFormNoteInput(e: WechatMiniprogram.CustomEvent) {
    this.setData({ formNote: (e.detail.value as string) || '' })
  },
